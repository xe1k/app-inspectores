...
(async () => {
  ...
  async function preparar(pagina, conSesion) {
    await pagina.evaluateOnNewDocument(() => localStorage.setItem('theme', 'dark'));
  ...
  await p.screenshot({ path: '_captura-login-dark-movil.png' });
  await p.close();
  ...
  await preparar(p, false);
  await p.goto('http://localhost:5174/login', { waitUntil: 'networkidle0', timeout: 60000 });
  await p.screenshot({ path: '_captura-login-dark-desktop.png' });
  await p.close();
  ...
  await p.screenshot({ path: '_captura-dashboard-dark-movil.png' });
  await p.close();
  ...
  await preparar(p, true);
  await p.goto('http://localhost:5174/', { waitUntil: 'networkidle0', timeout: 60000 });
  await p.screenshot({ path: '_captura-dashboard-dark-desktop.png' });
  await p.close();
